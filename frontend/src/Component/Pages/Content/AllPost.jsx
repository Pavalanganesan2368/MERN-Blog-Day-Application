...
const AllPost = () => {
  const { allPost, loading, searchAllPost, searchAllPostType, setSearchAllPostType } =
    useContext(DataContext);
  return (
    <main className="flex-1">
      <section className="flex flex-row gap-4 px-3 items-center justify-between bg-gray-800">
        <HeaderNav 
          setSearchAllPostType={setSearchAllPostType}
          searchAllPost={searchAllPost}
        />
      </section>

      <ul className="flex-1 flex-col h-100 gap-3 p-3 overflow-y-auto">
        {searchAllPost?.length === 0 ? (
